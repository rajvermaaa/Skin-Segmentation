...
image_names = ['test1.jpg', 'test2.jpg', 'test3.jpg', 'test4.jpg', 'test5.jpg']
path = "skin_test_imgs"
...
def load_all_test_images():
    ...
    skin_images = []       #This list will eventually hold all the loaded images.
    for index_image, name_image in enumerate(image_names):
        ...
        if not os.path.exists(image_path):
            print(f"Error: Image '{name_image}' not found at path '{image_path}'")
            continue
        
        # Read the image and add it (append) to the structure 'skin_images'
        img = cv2.imread(image_path)
        if img is None:
            print(f"Error: Unable to load image '{name_image}'")
            continue
...
def show_with_matplotlib(color_img, title, pos):
    ...
    ax = plt.subplot(5, 6, pos)   #creates a subplot using Matplotlib
    plt.imshow(img_RGB)
    plt.title(title)
    plt.axis('off')
...
# Apply the 'skin_detector' to all the images
